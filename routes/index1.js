              ...
              knex('delhi_teer').select('*').then(teer => {
                knex('image').select('*').orderBy('id', 'desc').limit('1').then(result_img => {
                  ...
                  if (teer[i].status == 'false') {
                    console.log("TERRRR",teer[i].status);
                    demo_date.push('Coming Soon');
                  } else {
                    // for (let i = 0; i < teer.length; i++) {
                    if(teer[i].value == "" || teer[i].value == null)  {
                      teer[i].value  = '**'
                    }else{
                      var win_num = teer[i].value.toString();
                      var teer_var = convert_teer(win_num);
                      demo_date.push(teer_var)
                    }
                    console.log("Coverrrrtttttteeerrrrr",convert_num_teer);
                  ...
                  var bahubali = [];
                  res.render('home_page', { details: details, value1: final_day, bahubali: bahubali, morning: morning, mornings: final_morning, night: night, nights: final_night, result_img: result_img, demo_date: demo_date });
...
router.get('/teer', function (req, res, next) {
  ...
  knex('delhi_teer').select('*').then(teer => {
    ...
    res.render('teer_pannel', { teer: teer, val: teer_arr })
  })
})
